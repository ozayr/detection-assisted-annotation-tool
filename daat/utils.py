...
def draw_info(class_map, zoombox, instance_buffer, detections):
    ...
    for line in static_string.split('\n'):
        ...
        if w > info_image.shape[1]:
            cv2.putText(info_image, line[:len(line) // 2], (10, (text_height + 1) * line_counter + 10), font,
                        font_scale, font_color, line_type)
            line_counter += 1
            line = line[len(line) // 2:]

        cv2.putText(info_image, line, (10, (text_height + 1) * line_counter + 10), font, font_scale, font_color,
                    line_type)
        line_counter += 1
    ...
    cv2.putText(info_image, detections, (10, (text_height + 1) * line_counter + 10), font, font_scale, font_color,
                line_type)
    line_counter += 1
    cv2.putText(info_image, label_status, (10, (text_height + 1) * line_counter + 10), font, font_scale, font_color,
                line_type)
...
def get_zoom_box(image, x, y, radius=100):
    ...
    zoombox = zoombox[zoombox.shape[0] // 2 - radius:zoombox.shape[0] // 2 + radius,
              zoombox.shape[1] // 2 - radius:zoombox.shape[1] // 2 + radius]
...
def voc2coco(annotations_dir, classes, do_augmentations, test_train_split=False, test_size=0.25):
    ...
    if (len(xml_files) >= int(1 / test_size)) and test_train_split:
        ...
        print(
            f'please annotate more than {int(1 / test_size)} images for the specified test_size of {test_size},coco conversion not done')
